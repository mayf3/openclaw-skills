...
class ReportValidator:
    ...
    def __init__(self, report_path: Path, quiet: bool = False):
    ...
    def _check_executive_summary(self) -> bool:
        """Check executive summary exists and is 200-400 words (warning: 100-199)"""
        ...
        if word_count < 100:
            self.errors.append(f"Executive summary too short: {word_count} words (should be ≥200, minimum 100)")
        elif word_count < 200:
            self.warnings.append(f"Executive summary shorter than ideal: {word_count} words (target 200-400)")
        ...
        if citation_nums:
            ...
            if missing:
                ...
                if len(missing_sorted) <= 10:
                    self.warnings.append(f"Non-consecutive citation numbers, missing: {missing_sorted}")
                else:
                    preview = missing_sorted[:10]
                    self.warnings.append(
                        f"Non-consecutive citation numbers, {len(missing_sorted)} missing: "
                        f"{preview} ... (and {len(missing_sorted) - 10} more)"
                    )
...
Examples:
  ...
  python validate_report.py -r reports/2026-0530-topic/00-executive-summary.md
        """
    )
    ...
    parser.add_argument(
        '--quiet', '-q',
        action='store_true',
        help='Quiet mode: only show summary, not individual checks'
    )
    ...
    validator = ReportValidator(report_path, quiet=args.quiet)
