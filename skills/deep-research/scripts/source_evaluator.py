...
class SourceEvaluator:
    ...
    def _evaluate_expertise(
        ...
        if author:
            if any(cred in author.lower() for cred in ['dr.', 'phd', 'professor', 'md']):
                score += 15
...
def _print_result(url: str, title: str, score: CredibilityScore):
    """Print formatted evaluation result"""
    print(f"\nSource: {title}")
    print(f"URL: {url}")
    print(f"Overall Score: {score.overall_score}/100")
    print(f"  Domain Authority: {score.domain_authority}/100")
    print(f"  Recency: {score.recency}/100")
    print(f"  Expertise: {score.expertise}/100")
    print(f"  Bias (neutrality): {score.bias_score}/100")
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate source credibility and quality",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python source_evaluator.py --url "https://www.nature.com/articles/s41586-2025-12345" --title "Breakthrough"
  python source_evaluator.py --url "https://example.com/article" --title "Some Article" --date "2025-03-15"
  python source_evaluator.py --demo

Tier mapping: high_trust (≥80) → T1-T2, moderate_trust (≥60) → T2-T3,
              low_trust (≥40) → T3-T4, verify (<40) → T4-T5
        """
    )
    parser.add_argument('--url', type=str, help='Source URL to evaluate')
    parser.add_argument('--title', type=str, default='', help='Article/page title')
    parser.add_argument('--date', type=str, default=None, help='Publication date (ISO format, e.g. 2025-06-01)')
    parser.add_argument('--author', type=str, default=None, help='Author name')
    parser.add_argument('--demo', action='store_true', help='Run demo with example sources')
    ...
    if args.demo or not args.url:
        # Demo mode
        print("Running demo with example sources...\n")
        test_sources = [
            {
        ...
        for source in test_sources:
            ...
            _print_result(source['url'], source['title'], score)
    else:
        ...
        _print_result(args.url, args.title or '(no title)', score)
