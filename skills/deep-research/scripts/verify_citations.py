...
class CitationVerifier:
    ...
    def __init__(self, report_path: Path, strict_mode: bool = False, quiet: bool = False):
    ...
    def verify_entry(self, entry: Dict) -> Dict:
        ...
        # STEP 2: Has DOI?
        if entry['doi']:
            self._log(f"  [{entry['num']}] Checking DOI {entry['doi']}...", end=' ')
            ...
            if success:
                ...
                self._log(f"✗ {metadata.get('error', 'Failed')}")
        ...
        if entry['url'] and result['status'] != 'verified':
            ...
            if url_ok:
                ...
                self._log(f"  [{entry['num']}] URL accessible ✓")
            else:
    ...
    def verify_all(self):
        ...
        self._log(f"\n{'='*60}")
        self._log(f"CITATION VERIFICATION: {self.report_path.name}")
        self._log(f"{'='*60}\n")

        entries = self.extract_bibliography()

        if not entries:
            self._log("❌ No bibliography entries found\n")
            return False

        self._log(f"Found {len(entries)} citations\n")
        ...
        self._log(f'DOI Verified: {len(verified)}/{len(results)}')
        self._log(f'URL Verified: {len(url_verified)}/{len(results)}')
        self._log(f'Suspicious: {len(suspicious)}/{len(results)}')
        self._log(f'Unverified: {len(unverified)}/{len(results)}')
        self._log("")

        if suspicious:
            self._log('SUSPICIOUS CITATIONS (Manual Review Needed):')
            for r in suspicious:
                self._log(f"\n  [{r['num']}]")
        ...
        if unverified and len(unverified) > 0:
            self._log('UNVERIFIED CITATIONS (Could not check):')
            for r in unverified:
                self._log(f"  [{r['num']}] {r['issues'][0] if r['issues'] else 'Unknown'}")
            self._log("")
        ...
        if suspicious:
            self._log('WARNING: Suspicious citations detected')
            if self.strict_mode:
                self._log('  STRICT MODE: Failing due to suspicious citations')
                return False
            else:
                self._log('  (Continuing in non-strict mode)')

        if self.strict_mode and unverified:
            self._log('STRICT MODE: Unverified citations found')
            return False

        if total_verified / len(results) < 0.5:
            self._log('WARNING: Less than 50% citations verified')
            return True  # Pass with warning
        else:
            self._log('CITATION VERIFICATION PASSED')
            return True
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  python verify_citations.py --report reports/2026-0530-topic/02-deep-dive.md
...
Uses free DOI resolver - no API key needed.
    ...
    verifier = CitationVerifier(report_path, strict_mode=args.strict, quiet=args.quiet)
